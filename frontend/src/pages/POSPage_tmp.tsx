...
const existing = cart.find(i => i.product.id === product.id)
  ...
  async function setQtyDirect(productId: string, qty: number) {
    if (!saleId) return
    const product = cart.find(i => i.product.id === productId)?.product
    const maxStock = product?.current_stock ?? qty
    const clamped = Math.min(Math.max(1, qty), maxStock)
    try {
  ...
  async function finalize(method: 'cash' | 'card') {
    ...
    try {
      ...
      showStatus(`✓ Sale finalized — ${method.toUpperCase()} ${cartTotal().toLocaleString('hu-HU')} Ft`, 'success')
      await startNewSale()
      await loadProducts()
    } catch {
      ...
      <div className="w-80 bg-white shadow-lg flex flex-col">
            ...
            <div key={item.product.id} className="flex justify-between items-center gap-2">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400">
                  {Number(item.product.current_price).toLocaleString('hu-HU')} Ft each
                </p>
              </div>
              {/* Quantity stepper */}
              <div className="flex items-center gap-1">
                ...
                <input
                  ...
                  onChange={e => {
                    ...
                    if (!isNaN(val) && val > 0) setQtyDirect(item.product.id, val)
                  }}
                  onKeyDown={e => { if (e.key === 'Enter') e.currentTarget.blur() }}
                  className="w-10 text-center text-sm font-medium border border-gray-200 rounded focus:outline-none focus:border-blue-400"
                />
                <button
                  onClick={() => updateQty(item.product.id, +1)}
                  className="w-6 h-6 rounded bg-gray-100 hover:bg-gray-200 text-gray-600 font-bold text-sm flex items-center justify-center"
              ...
              <p className="text-sm font-bold w-14 text-right">
                {(Number(item.product.current_price) * item.quantity).toLocaleString('hu-HU')} Ft
              </p>
              <button
        ...
        <div className="p-4 border-t">
          <div className="flex justify-between mb-4">
            ...
            <span className="font-bold text-lg">{cartTotal().toLocaleString('hu-HU')} Ft</span>
          </div>
